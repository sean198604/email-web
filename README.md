...
- **邮件追踪**：HTML 邮件自动注入打开像素并改写外链，可按任务和联系人查看打开、点击、最近互动时间与最近点击链接。
...
| `APP_URL` | 邮件追踪建议设置 | 收件人能访问的应用公网根地址，例如 `https://email.pocketbay.app`；不要以 `/` 结尾 |
...
6. 在 **邮件追踪** 中按任务查看打开率、点击率和联系人互动明细。

## 👁️ 邮件追踪

发送 HTML 邮件时，系统会自动加入一个透明打开像素，并将其中的 HTTP/HTTPS 外链改为先记录点击、再跳转至原链接的地址。登录后可在 **邮件追踪** 页面按任务筛选，查看：

- 成功发送数、打开数与打开率；总点击数、点击人数与点击率；
- 各群发任务的打开 / 点击对比；
- 各联系人的打开次数、最后打开时间、点击次数、最后点击时间与最近点击链接。

部署到公网时，请将 `APP_URL` 设置为收件人可访问的 HTTPS 根地址；否则邮件客户端无法请求追踪像素或跳转链接。该功能仅适用于 HTML 邮件。部分客户端会拦截远程图片、使用图片代理或预加载链接，因此统计数据应作为互动信号而非绝对阅读事实。使用前请向收件人提供适当告知，并遵守适用的隐私与反垃圾邮件法规。
...
- 邮件追踪会记录发送记录关联的打开/点击时间和最近点击链接；请按适用法律完成告知、授权与数据保留管理。
